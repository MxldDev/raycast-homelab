...
function qbitPrefs() {
  return {
    url: requireUrl("qbitUrl", "qBittorrent"),
    username: setting("qbitUsername") || "admin",
    password: setting("qbitPassword"),
  };
}
...
async function qbitLogin(): Promise<string> {
  ...
  if (
    qbitCookie &&
    qbitCookie.host === url &&
    Date.now() - qbitCookie.at < 30 * 60 * 1000
  ) {
  ...
  if (!res.ok || body.trim() === "Fails.")
    throw new Error(`qBittorrent login failed (${res.status} ${body.trim()})`);
...
async function qbitGet<T>(path: string): Promise<T> {
  ...
  if (res.status === 403) {
    ...
    if (!retry.ok)
      throw new Error(`qBittorrent ${path} → HTTP ${retry.status}`);
...
export async function qbitToggle(
  hash: string,
  action: "stop" | "start",
): Promise<void> {
  const { url } = qbitPrefs();
...
function sabPrefs() {
  return {
    url: requireUrl("sabnzbdUrl", "SABnzbd"),
    key: setting("sabnzbdApiKey"),
  };
}
...
async function sabGet<T>(params: Record<string, string>): Promise<T> {
  ...
  const res = await fetch(`${url}/api?${qs}`, {
    signal: AbortSignal.timeout(TIMEOUT_MS),
  });
...
export async function sabToggle(
  nzoId: string,
  action: "pause" | "resume",
): Promise<void> {
...
async function loadSlskd(): Promise<
  { items: DownloadItem[]; dlSpeed: number } | undefined
> {
  if (!has("slskdUrl", "slskdApiKey")) return undefined;
  const url = requireUrl("slskdUrl", "slskd");
  const res = await fetch(`${url}/api/v0/transfers/downloads`, {
    headers: { "X-API-Key": setting("slskdApiKey") },
    signal: AbortSignal.timeout(TIMEOUT_MS),
  });
    ...
    for (const dir of u.directories ?? []) {
      for (const f of dir.files ?? []) {
        ...
        items.push({
          ...
          speed:
            active && f.averageSpeed > 0 ? fmtSpeed(f.averageSpeed) : undefined,
  ...
  items.sort(
    (a, b) =>
      (a.state === "downloading" ? -1 : 1) -
      (b.state === "downloading" ? -1 : 1),
  );
...
export async function loadDownloads(): Promise<DownloadsData> {
  ...
  // after 5 failed attempts, and behind a reverse proxy that bans the whole vhost.
  if (!has("qbitUrl")) {
    // not configured: skip silently
  } else if (!has("qbitPassword")) {
    data.qbitHint =
      "qBittorrent password not set — press ⌘K → Configure Extension";
  } else {
    tasks.push(
      Promise.all([
  ...
  if (has("sabnzbdUrl", "sabnzbdApiKey"))
    tasks.push(
